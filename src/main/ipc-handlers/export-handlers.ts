...
export class ExportHandlers {
  ...
  setupHandlers(): void {
    ...
    ipcMain.handle('generate-capture-one-style', async (_event, data: { adjustments: any; styleName?: string; description?: string }) => {
      try {
        const styleName = data.styleName || 'Custom Style';
        const description = data.description || 'Generated by Film Recipe Wizard';
        const styleContent = generateCaptureOneStyle(data.adjustments, styleName, description);
        return { success: true, content: styleContent };
      } catch (error) {
        console.error('[IPC] Error generating Capture One style:', error);
        return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
      }
    });

    // Handle Capture One style download - generate style and show save dialog
    ipcMain.handle('download-capture-one-style', async (_event, data) => {
      try {
        const styleName = (data.styleName as string) || 'Custom Style';
        const description = (data.description as string) || 'Generated by Film Recipe Wizard';
        ...
        const result = await dialog.showSaveDialog({
          ...
          defaultPath: `${safeName}.costyle`,
          filters: [
            { name: 'Capture One Styles', extensions: ['costyle'] },
            { name: 'All Files', extensions: ['*'] },
          ],
        });
        ...
        if (!result.canceled && result.filePath) {
          // Write the file
          await fs.writeFile(result.filePath, styleContent, 'utf8');
          return { success: true, filePath: result.filePath };
        } else {
          return { success: false, error: 'Save canceled' };
        }
      } catch (error) {
        console.error('[IPC] Error downloading Capture One style:', error);
        return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
      }
    });
  }
