...
const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  const handleExportCaptureOne = async (result: ProcessingResult) => {
    const adjustments = getEffectiveAdjustments(result);
    if (!adjustments) return;
    ...
    try {
      ...
      const res = await window.electronAPI.downloadCaptureOneStyle({
        adjustments: adjForExport,
        styleName: processName || 'Custom Style',
        description: processDescription || 'Generated by Film Recipe Wizard',
      });
      if (!res.success) {
        showError(`Capture One export failed: ${res.error}`);
      }
    } catch (e) {
      console.error('Capture One export error:', e);
      showError('Capture One export failed.');
    }
  };
      ...
      {successfulResults.length > 0 && (
        <Paper className="card" sx={{ p: 0, overflow: 'hidden' }}>
          ...
          >
            ...
            <Tab icon={<CameraAltIcon />} label="Capture One Export" iconPosition="start" />
                      ...
                      <Paper elevation={1} sx={{ p: 4 }}>
                        ...
                        <Box sx={{ mb: 3 }}>
                          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 1, color: 'primary.main' }}>
                            How to Use Your Preset
                          </Typography>
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            1. Import the .xmp file into Lightroom Classic using the Presets panel
                          </Typography>
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            2. Apply the preset to any image by clicking on it in the Presets panel
                          </Typography>
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            3. Fine-tune individual adjustments as needed for each image
                          </Typography>
                        </Box>
                        ...
                        >
                          ...
                          <Typography variant="body2">
                            Lightroom presets preserve the exact color transformations from your AI recipe, 
                            making them perfect for maintaining consistency across a photo series.
                          </Typography>
                        </Box>
                      </Paper>
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                      <Paper elevation={1} sx={{ p: 4 }}>
                        ...
                        <Typography variant="body2" sx={{ color: 'text.secondary', mb: 3 }}>
                          Create a Capture One style file from your recipe adjustments for use in Capture One Pro.
                        </Typography>
                        
                        <Box sx={{ mb: 3 }}>
                          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 1, color: 'primary.main' }}>
                            What is a Capture One Style?
                          </Typography>
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            A Capture One style file contains all the color adjustments, tone mapping, and creative effects 
                            from your recipe, allowing you to apply the same look to other images in Capture One Pro.
                          </Typography>
                        </Box>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                          <Button
                            variant="contained"
                            startIcon={<DownloadIcon />}
                            onClick={() => handleExportCaptureOne(result)}
                            sx={{ textTransform: 'none', fontWeight: 700, py: 2, px: 4 }}
                            size="large"
                          >
                      ...
                      {/* Capture One Information */}
                      <Paper elevation={1} sx={{ p: 4 }}>
                        <Divider sx={{ mb: 3 }} />
                        <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>
                          About Capture One Styles
                        </Typography>
                        ...
                        <Box sx={{ mb: 3 }}>
                          ...
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            1. Import the .costyle file into Capture One Pro using the Styles and Presets tool
                          </Typography>
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            2. Apply the style to any image by clicking on it in the Styles panel
                          </Typography>
                          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                            3. Fine-tune individual adjustments as needed for each image
                          </Typography>
                        ...
                        >
                          ...
                          <Typography variant="body2">
                            Capture One styles preserve the exact color transformations from your AI recipe, 
                            making them perfect for maintaining consistency across a photo series.
                          </Typography>
                        </Box>
                      </Paper>
