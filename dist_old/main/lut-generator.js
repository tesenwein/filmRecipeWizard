...
exports.generateLUTContent = generateLUTContent;
function generateLUTContent(aiAdjustments, size = 33, format = 'cube') {
    // Sanitize and clamp adjustment values with sensible defaults
    const isNum = (v) => typeof v === 'number' && Number.isFinite(v);
    const clamp = (v, min, max) => Math.max(min, Math.min(max, v));
    const withDefault = (v, def) => (isNum(v) ? v : def);
...
function generateCubeLUT(adjustments, size) {
...
function generate3DLLUT(adjustments, size) {
    let lut = `3DMESH
Mesh ${size} ${size} ${size}
...
function generateDaVinciLUT(adjustments, size) {
    let lut = `# Created by Film Recipe Wizard
# DaVinci Resolve LUT
...
function applyColorTransform(r, g, b, A) {
    ...
    const wb = [tgt[0] / ref[0], tgt[1] / ref[1], tgt[2] / ref[2]];
    ...
    if (Math.abs(A.hue_red) + Math.abs(A.hue_orange) + Math.abs(A.hue_yellow) + Math.abs(A.hue_green) +
        ...
        const hueShiftDeg = (w.red * A.hue_red + w.orange * A.hue_orange + w.yellow * A.hue_yellow + w.green * A.hue_green +
            w.aqua * A.hue_aqua + w.blue * A.hue_blue + w.purple * A.hue_purple + w.magenta * A.hue_magenta) * 0.5; // max ~50°
        const satMul = 1 + (w.red * A.sat_red + w.orange * A.sat_orange + w.yellow * A.sat_yellow + w.green * A.sat_green +
            w.aqua * A.sat_aqua + w.blue * A.sat_blue + w.purple * A.sat_purple + w.magenta * A.sat_magenta) * 0.6; // max +/-60%
        const lumAdd = (w.red * A.lum_red + w.orange * A.lum_orange + w.yellow * A.lum_yellow + w.green * A.lum_green +
            w.aqua * A.lum_aqua + w.blue * A.lum_blue + w.purple * A.lum_purple + w.magenta * A.lum_magenta) * 0.15; // gentle +/-0.15
    ...
    if (A.shadowSat > 1e-3 || A.midtoneSat > 1e-3 || A.highlightSat > 1e-3 || A.globalSat > 1e-3) {
        ...
        const applyWheel = (w, hueDeg, sat, lum) => {
            if (w < 1e-3 || sat < 1e-3)
                return;
...
function hueBandWeights(hDeg) {
    ...
    const sum = Object.values(weights).reduce((a, v) => a + v, 0) || 1;
