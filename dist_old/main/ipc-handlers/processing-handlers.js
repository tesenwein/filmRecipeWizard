...
exports.ProcessingHandlers = void 0;
const electron_1 = require("electron");
const fs = __importStar(require("fs/promises"));
const lut_generator_1 = require("../lut-generator");
class ProcessingHandlers {
    constructor(getMainWindow, imageProcessor, storageService) {
    ...
    setupHandlers() {
        ...
        electron_1.ipcMain.handle('generate-preview', async (_event, args) => {
            try {
        ...
        electron_1.ipcMain.handle('process-images', async (_event, data) => {
        ...
        electron_1.ipcMain.handle('process-with-stored-images', async (_event, data) => {
            const mainWindow = this.getMainWindow();
            if (!mainWindow || !this.storageService)
                return [];
            try {
                ...
                const baseImageData = (() => {
                    if (Array.isArray(data.baseImageData))
                        return data.baseImageData.slice(0, 3);
                    if (typeof data.baseImageData === 'string')
                        return data.baseImageData;
                    return undefined;
                })();
                ...
                const options = data.styleOptions || stored?.userOptions || {};
                ...
                if (options.lightroomProfile !== undefined) {
                    const profileName = options.lightroomProfile === 'adobe-color'
                        ? 'Adobe Color'
                ...
                if (options.artistStyle && typeof options.artistStyle.name === 'string') {
                    ...
                    optionsHintParts.push(`Artist Style: ${name}${category ? ` (${category})` : ''}` +
                        (blurb ? `\nNotes: ${blurb}` : ''));
                }
                if (options.filmStyle && typeof options.filmStyle.name === 'string') {
                    ...
                    optionsHintParts.push(`Film Stock: ${name}${category ? ` (${category})` : ''}` +
                        (blurb ? `\nTraits: ${blurb}` : ''));
                }
                ...
                try {
                    ...
                    let derivedName;
                    try {
                        const aiName = result?.metadata?.aiAdjustments?.preset_name;
                        if (aiName && typeof aiName === 'string' && aiName.trim().length > 0) {
                            const extras = [];
                            const artist = options?.artistStyle?.name;
                            const film = options?.filmStyle?.name;
                            if (artist && artist.trim().length > 0)
                                extras.push(artist.trim());
                            if (film && film.trim().length > 0)
                                extras.push(film.trim());
        ...
        electron_1.ipcMain.handle('generate-lut', async (_event, data) => {
            try {
                ...
                const lutContent = (0, lut_generator_1.generateLUTContent)(adjustments, data.size, data.format);
                // Show save dialog
                const sanitizeName = (n) => n
                    .replace(/\b(image\s*match|imagematch|match|target|base|ai|photo)\b/gi, '')
                    .replace(/\s{2,}/g, ' ')
                    .trim();
                const rawName = data?.adjustments?.preset_name ||
...
exports.ProcessingHandlers = ProcessingHandlers;
